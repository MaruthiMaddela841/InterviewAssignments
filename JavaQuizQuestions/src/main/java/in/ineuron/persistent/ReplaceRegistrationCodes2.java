...
public class ReplaceRegistrationCodes2 {
    public static void main(String[] args) throws Exception {
        ...
        //List<CodeExample> examples = mapper.readValue(json, new TypeReference<List<CodeExample>>() {});
        CodeExample[] exampleArray = mapper.readValue(json, CodeExample[].class);
        List<CodeExample> examples = Arrays.asList(exampleArray);
        
        
        for(CodeExample ex:examples) {
        	String code=ex.getRegistration();
        	if(regMap.containsKey(code)) {
        		ex.setRegistration(regMap.get(code));
        	}
        }

        // Print updated JSON
        String updatedJson = mapper.writerWithDefaultPrettyPrinter().writeValueAsString(examples);
